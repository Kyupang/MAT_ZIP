...
public class ReviewController {
	...
	public String insertReview(ReviewVO vo, @RequestParam("selectedReceipt") String selectedReceipt, HttpSession session) {
		
		System.out.println("selectedReceipt: " + selectedReceipt);  // 이 부분을 추가
		...
		// 사용자가 영수증을 선택하지 않은 경우에 대한 예외 처리
	    if (selectedReceipt == null || selectedReceipt.isEmpty()) {
	        // 에러 메시지를 세션에 추가하고 리뷰 작성 페이지로 리다이렉션
	        session.setAttribute("errorMessage", "영수증을 선택해주세요.");
	        return "redirect:/writeReview";
	    }
	    
	    
	    int receiptId = Integer.parseInt(selectedReceipt); // 문자열 형태의 영수증 ID를 int로 변환
	    vo.setReceipt_id(receiptId); // 변환한 영수증 ID를 vo에 설정
